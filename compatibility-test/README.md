...
> **Current limitation (October 2025):** the collector pushes only `event.data.event` into the buffer (see `runCase.ts`), while `testEvents` still looks for the outer `raw_model_stream_event` wrapper. That mismatch means Responses streaming runs currently report `validEvents: false` even when the backend emits the right delta/done sequence. Fixing this will require changing the collector, which we are tracking separately from this README update.
...
- While iterating over the stream, the harness currently pushes `event.data.event` into an internal buffer: these are the raw OpenAI model events after stripping the transport wrapper. Because we drop the wrapper, `testEvents` later fails to see the original `raw_model_stream_event` type—see limitation note above.
...
### What counts as a failure
- **Missing reasoning events:** `testEvents` expects every Responses run to emit both `response.reasoning_text.delta` and `response.reasoning_text.done`. Chat runs must surface at least one `choices[0].delta.reasoning` string. Absence of these flips `validEvents` to `false`. Because of the wrapper mismatch mentioned above, Responses streams currently fail this check even when the provider emits the right events.
...
| `--verbose` | `-v` | `false` | Writes a detailed run log to `log_<provider>_<timestamp>.log`. |
