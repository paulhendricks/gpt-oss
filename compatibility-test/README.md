...
2. Confirms that non-streaming ("unary") responses include the expected reasoning payload for each API type (Responses and Chat Completions). When a Responses run only supplies a `summary` array, the harness records a warning and still counts the run as valid so long as the array is present.
...
- **Malformed reasoning payload:** `testOutputData` expects unary Chat runs to include at least one assistant message with a non-empty `reasoning` string, and unary Responses runs to emit a `reasoning` output item with either populated `reasoning_text` entries or a `summary` array. If only a summary array is present, the harness marks the case as passed but records a warning in the summary details.
...
- **Malformed event shapes:** Events missing the nested `.data.event.type` field are treated as unknown and logged in the summary. When no recognizable reasoning events are present the case fails even if the rest of the run succeeds. As with unary runs, a Responses stream that delivers only a summary array is accepted but annotated with a warning.
...
- Node.js 18 or newer (for the TypeScript harness).
- tsx `npm install -g tsx` (or local `npm install --save-dev tsx`)
...
| `--streaming` | — | `false` | When set, requests streaming results from the provider and enables event validation. Leave unset for unary runs. |
...
| `--strict` | — | `false` | Enforces an exact match between emitted tool arguments and `expected_arguments`. |
| `--verbose` | `-v` | `false` | Writes a detailed run log to `log_<provider>_<timestamp>.log`, including every `runCase` reasoning/tool-call diagnostic. |

`--streaming` and `--strict` intentionally omit short aliases so both flags can be specified together without collisions.

When `--verbose` is enabled, the harness records runtime configuration, raw case payloads, run summaries, and error stacks in the log file alongside the standard console output.
...
- `log_<provider>_<timestamp>.log` — optional verbose trace created when you pass `--verbose`.
